...
#define EVENTID "Raspberry Pi Sense HAT Joystick"
    int result;
    int i;
    glob_t globbuf;
    bool foundEvent;
    ...
    result = glob(EVENTPATH, NULL, NULL, &globbuf);
    ...
    while(globbuf.gl_pathv[i]){
        ...
        result = ioctl(eventFd, EVIOCGNAME(sizeof(name)), &name);
        if (result < 0){
            close(eventFd);
            i++;
            continue;
...
bool initializeSenseHat() {
    ...
    result = glob(PATH, NULL, NULL, &globbuf);
    ...
    while(globbuf.gl_pathv[i]){
        fd = open(globbuf.gl_pathv[i], O_RDWR);
        if(fd < 0){
            close(fd);
            i++;
            continue;
        }
        result = ioctl(fd, FBIOGET_FSCREENINFO, &fb_info);
        if (result < 0){
            close(fd);
            i++;
            continue;
        }
        if(strcmp(fb_info.id, ID) == 0){
            foundFb = true;
            break;
        }
        i++;
...
uint16_t getRandomBytes(){
    int fd;
    ssize_t read_in;
    char *return_value = malloc(16);
    
    fd = open("/dev/urandom", O_RDONLY);
    if (fd < 0) return 0;
    read_in = read(fd, return_value, 16);
    if(read_in != 16) return 0;
...
static inline void newTile(coord const target) {
  ...
  game.playfield[target.y][target.x].color = getRandomBytes();
}
